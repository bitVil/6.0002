...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        self.width = width
        self.height = height
        self.num_cleaned_tiles = 0
        self.tiles = []
        for i in range(width):
            tile_row = []
            for i in range(height):
                tile_row.append(dirt_amount)
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        if (self.tiles[x][y] > 0 and self.tiles[x][y] - capacity > 0):
            self.tiles[x][y] -= capacity
        elif (self.tiles[x][y] > 0 and self.tiles[x][y] - capacity <= 0):
            self.tiles[x][y] = 0
            self.num_cleaned_tiles += 1
    ...
    def is_position_in_room(self, pos):
        ...
        """
        xPos = math.floor(pos.get_x())
        yPos = math.floor(pos.get_y())
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for i in range(num_trials):
        ...
        dirty_tiles = width * height
        clock_ticks = 0
        for i in range(num_robots):
            robot_list.append(robot_type(room, speed, capacity))
        # Do another time step if proportion of
        # dirty tiles is > min_coverage.
        while dirty_tiles /(width * height) > min_coverage:
            # have robots go through a time step.
            for robot in robot_list:
                robot.update_position_and_clean()
            clock_ticks += 1
            dirty_tiles = 0
            # Count dirty tiles left.
            for j in range(height):
                for k in range(width):
                    if not room.is_tile_cleaned(k, j):
                        dirty_tiles += 1
            clock_ticks += 1
        times_list.append(clock_ticks)
